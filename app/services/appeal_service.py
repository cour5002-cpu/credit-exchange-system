...
from app.services.appeal_lifecycle import notify_appeal_completed
...
def admin_approve_appeal(user, appeal_id, admin_advice):
    ...
    create_notification(
        appeal.applicant.user_id, user.id, "appeal", "appeal_reopened",
        "申诉已通过并进入重新审核",
        f"您的申诉“{appeal.appeal_no}”已通过管理员认定，原业务已进入重新审核流程。",
        biz_type="appeal", biz_id=appeal.id,
        payload={"status": appeal.status, "reopen_stage": appeal.reopen_stage},
        dedupe_key=f"appeal:{appeal.id}:1:reopened:student",
    )
    advisor_user_id = _appeal_advisor_user_id(appeal, target)
    if advisor_user_id:
        create_notification(
            advisor_user_id, user.id, "appeal", "appeal_advisor_reconfirmation_requested",
            "申诉业务待再次确认",
            f"申诉“{appeal.appeal_no}”已由管理员通过，请对原业务再次确认。",
            biz_type="appeal", biz_id=appeal.id,
            payload={"target_type": appeal.target_type, "target_id": appeal.target_id, "reopen_stage": appeal.reopen_stage},
            dedupe_key=f"appeal:{appeal.id}:1:advisor-reconfirmation:requested",
        )
    db.session.commit()
...
def advisor_reconfirm_appeal(user, appeal_id, decision, comment=None):
    ...
    if appeal.status == "completed":
        notify_appeal_completed(appeal, user.id)
    db.session.commit()
    return appeal, target
...
def list_reopened_pending_final(page=None, page_size=None):
    reconcile_reopened_hour_appeals()
    query = (
        Appeal.query.join(HourApplication, HourApplication.id == Appeal.target_id)
        .filter(
            Appeal.status == "processing",
            Appeal.reopen_stage == "pending_admin_final",
            Appeal.target_type == "hour_application",
...
def _appeal_advisor_user_id(appeal, target):
    if appeal.target_type == "hour_application":
        link = ApplicationAdvisor.query.filter_by(
            application_id=target.id,
            advisor_role="primary",
            can_operate=True,
        ).first()
        return link.teacher.user_id if link and link.teacher else None
    return target.advisor.user_id if target.advisor else None
