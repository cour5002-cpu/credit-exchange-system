...
from app.services.admin_batch_service import batch_approve_task_publish_requests
...
def advisor_task_registrations(task_id):
    return _handle_business(lambda: ok({"items": [
        _task_registration_summary(item, include_contact_phone=True)
        for item in list_task_registrations_for_advisor(current_user, task_id)
    ]}))
...
def student_task_detail(task_id):
    return _handle_business(lambda: ok(_student_task_detail_payload(
        *get_student_task(current_user, task_id), user=current_user
    )))
...
def student_task_registration_detail(registration_id):
    ...
    return _handle_business(lambda: ok({
        "registration": _task_registration_summary(registration, include_contact_phone=True),
...
@api_bp.route("/admin/task-publish-requests/batch-approve", methods=["POST"])
@login_required
@role_required("admin")
def admin_batch_approve_task_publish_api():
    return _handle_business(lambda: ok(batch_approve_task_publish_requests(
