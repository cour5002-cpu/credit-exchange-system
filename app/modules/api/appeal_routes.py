...
@api_bp.route("/admin/appeals/reopened/pending-final", methods=["GET"])
@login_required
@role_required("admin")
def admin_reopened_pending_final():
    return _handle_business(lambda: _paged_response(
        lambda page, page_size: list_reopened_pending_final(page, page_size),
        _appeal_summary,
    ))


@api_bp.route("/admin/appeals/reopened/batch-assign-reviewer", methods=["POST"])
@login_required
@role_required("admin")
def admin_batch_assign_reopened_appeals():
    return _handle_business(lambda: ok(batch_assign_reopened_appeal_reviewer(
        current_user, request.get_json(silent=True)
    )))


@api_bp.route("/admin/appeals/reopened/batch-final-approve", methods=["POST"])
@login_required
@role_required("admin")
def admin_batch_final_approve_reopened_appeals():
    return _handle_business(lambda: ok(batch_final_approve_reopened_appeals(
        current_user, request.get_json(silent=True)
    )))
