...
from app.schemas.extension_rule import public_extension_rule
...
def extension_created_payload(extension):
    ...
    return {
        ...
        "rule_id": extension.rule_id,
        "rule_version": (extension.rule_snapshot or {}).get("rule_version"),
        "approved_extension_days_before": extension.approved_extension_days_before,
        "projected_total_extension_days": extension.projected_total_extension_days,
    }
...
def extension_request_payload(extension):
    ...
    return {
        ...
        "review_level": extension.review_level,
        "rule_id": extension.rule_id,
        "rule_snapshot": extension.rule_snapshot,
        "approved_extension_days_before": extension.approved_extension_days_before,
        "projected_total_extension_days": extension.projected_total_extension_days,
        "status": extension.status,
...
def extension_eligibility_payload(result):
    expected_graduation_date = result["expected_graduation_date"]
    return {
        "can_apply": result["can_apply"],
        "reason_code": result["reason_code"],
        "reason": result["reason"],
        "application_id": result["application_id"],
        "current_due_at": iso(result["current_due_at"]),
        "submitted_request_count": result["submitted_request_count"],
        "approved_extension_days": result["approved_extension_days"],
        "remaining_request_count": result["remaining_request_count"],
        "remaining_extension_days": result["remaining_extension_days"],
        "expected_graduation_date": expected_graduation_date.isoformat()
        if expected_graduation_date else None,
        "rule": public_extension_rule(result["rule"]),
    }
