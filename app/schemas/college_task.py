...
from app.core.validators import MAINLAND_MOBILE_PATTERN, is_mainland_mobile
...
def task_detail_payload(task):
    return {
        ...
        "registrations": [
            task_registration_summary(item, include_contact_phone=True)
...
def student_task_detail_payload(task, registration, user=None):
    ...
    payload["registration"] = (
        task_registration_summary(registration, include_contact_phone=True)
        if registration else None
    )
    default_phone = getattr(user, "phone", None) if user else None
    payload["registration_form"] = {
        "contact_phone_required": True,
        "default_contact_phone": default_phone.strip()
        if is_mainland_mobile(default_phone) else None,
        "contact_phone_pattern": MAINLAND_MOBILE_PATTERN,
...
def task_registration_summary(registration, include_contact_phone=False):
    if not registration:
        return None
    item = {
        "id": registration.id,
    ...
    if include_contact_phone:
        item["contact_phone"] = registration.contact_phone
    return item
