...
const pending = computed(() => result.value?.actions?.can_approve===true||result.value?.actions?.can_reject===true||(result.value?.canOperate===true&&result.value?.status==='material_submitted')||result.value?.status==='material_submitted')
const resultStatusText = computed(() => ({ material_submitted: '待指导老师确认成果', advisor_rejected: '成果已驳回', pending_assignment: '待管理员分配审核老师' }[result.value?.status] || result.value?.status))
...
<template><main class="page"><div class="content"><template v-if="result">
  <header><div><p class="eyebrow">T107 · RESULT DETAIL</p><h1>任务成果确认详情</h1><p>{{ result.resultId }}</p></div><StatusTag :status="result.status" :text="resultStatusText" /></header>
  <section class="card"><h2>任务基本信息</h2><dl class="grid"><div><dt>任务名称</dt><dd>{{ result.taskTitle }}</dd></div><div><dt>任务编号</dt><dd>{{ result.taskId }}</dd></div><div><dt>任务类型</dt><dd>{{ task ? getTaskTypeText(task.taskType) : '--' }}</dd></div><div><dt>指导老师</dt><dd>{{ result.advisorName }}</dd></div><div><dt>成果提交时间</dt><dd>{{ result.submitTime }}</dd></div><div><dt>当前成果状态</dt><dd><StatusTag :status="result.status" :text="resultStatusText" /></dd></div></dl></section>
  ...
  <section class="card"><h2>成果说明</h2><dl class="grid"><div><dt>申请课时</dt><dd>{{ result.requestedHours }}</dd></div><div class="wide"><dt>成果说明</dt><dd class="description">{{ result.resultDescription }}</dd></div></dl></section>
