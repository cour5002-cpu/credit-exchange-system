...
async function loadItems(){loading.value=true;try{const adapted=adaptApplicationList(await getPendingFinalApplications({page:page.value,page_size:pageSize.value}));realItems.value=adapted.items;total.value=Number(adapted.total??adapted.items.length);page.value=Number(adapted.page??page.value);pageSize.value=Number(adapted.page_size??pageSize.value)}catch(error){window.alert(getApiErrorMessage(error,'待最终确认申请加载失败'))}finally{loading.value=false}}
onMounted(loadItems)
