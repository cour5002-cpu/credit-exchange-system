...
<template><main class="detail-page"><div class="detail-content"><template v-if="item">
  ...
  <ReviewActionBar v-if="!readonly && item.status === APPLICATION_STATUS.PENDING_REVIEWER" :approve-text="hoursChanged ? '修改课时并通过' : '审核通过'" reject-text="审核驳回" @approve="hoursChanged ? approveWithChange() : approve()" @reject="reject"><template #before><button class="back-button" type="button" @click="goBack">返回</button></template></ReviewActionBar>
