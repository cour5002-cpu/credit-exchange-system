...
import { EXCHANGE_STATUS, finalApproveExchange, finalRejectExchange, getExchanges } from '../mock/exchanges.js'
...
function approve() {
  const approved = item.value && finalApproveExchange(item.value.id, opinion.value.trim())
  if (!approved) return window.alert('当前申请不满足最终确认兑换条件，请检查课时到账、兑换状态、指导老师确认状态和成员分配表。')
