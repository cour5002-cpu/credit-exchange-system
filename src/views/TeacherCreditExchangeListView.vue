...
import { EXCHANGE_STATUS, getAdvisorPendingExchanges } from '../mock/exchanges.js'

const currentAdvisorId = 'T001'
const keyword = ref('')
const selectedStatus = ref(EXCHANGE_STATUS.PENDING_CONFIRMATION)
const items = computed(() => {
  const search = keyword.value.trim().toLowerCase()
  return getAdvisorPendingExchanges(currentAdvisorId).filter((item) => {
...
<template><main class="page"><div class="content">
  ...
  <section class="filters"><label><span>搜索</span><input v-model="keyword" type="search" placeholder="编号、项目、任务、队长或团队" /></label><label><span>状态</span><select v-model="selectedStatus"><option value="">全部状态</option><option :value="EXCHANGE_STATUS.PENDING_CONFIRMATION">待指导老师确认</option></select></label></section>
