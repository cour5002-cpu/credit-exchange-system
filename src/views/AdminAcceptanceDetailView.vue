...
function rejectApplication() { if (!opinion.value.trim()) { feedback.value = { type: 'error', message: '请填写驳回原因' }; window.alert(feedback.value.message); return } item.value.status = 'rejected'; feedback.value = { type: 'success', message: '驳回成功' }; window.alert(feedback.value.message); goBack() }
...
<template>
  <main class="detail-page"><div class="detail-content">
    <template v-if="item">
      ...
      <ReviewActionBar approve-text="受理并分配" reject-text="驳回" :disabled="item.status !== APPLICATION_STATUS.PENDING_ADMIN_ACCEPT" @approve="acceptApplication" @reject="rejectApplication"><template #before><button class="back-button" type="button" @click="goBack">返回</button></template></ReviewActionBar>
    </template>
