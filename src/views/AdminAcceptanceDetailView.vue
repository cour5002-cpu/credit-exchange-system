...
<template>
  <main class="detail-page"><div class="detail-content">
    <template v-if="item">
      ...
      <section class="card"><h2>团队成员</h2><dl class="info-grid team-summary"><div><dt>团队名称</dt><dd>{{ item.team?.name || item.teamName || '--' }}</dd></div><div><dt>队长</dt><dd>{{ item.team?.leader?.name || item.leader?.name || '--' }}</dd></div></dl><div class="table-wrapper"><table><thead><tr><th>姓名</th><th>学号</th><th>学院</th><th>专业</th><th>角色</th></tr></thead><tbody><tr v-for="member in item.members" :key="member.id"><td>{{ member.name }}</td><td>{{ member.studentId }}</td><td>{{ member.college || '--' }}</td><td>{{ member.major || '--' }}</td><td>{{ member.role === 'captain' ? '队长' : '成员' }}</td></tr><tr v-if="!item.members.length"><td colspan="5" class="empty">详情接口未返回团队成员</td></tr></tbody></table></div></section>
