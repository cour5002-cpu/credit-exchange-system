...
function acceptApplication() {
  ...
  feedback.value = { type: 'success', message: `受理并分配成功，已分配给${acceptedApplication.reviewer.name}老师。` }
  window.alert(feedback.value.message)
  goBack()
}
...
<template>
  <main class="detail-page"><div class="detail-content">
    <template v-if="item">
      <header class="page-header"><div><p class="eyebrow">ACCEPTANCE DETAIL</p><h1>{{ item.title }}</h1><p>{{ item.applyTypeText }}</p></div><StatusTag :status="item.status" :text="item.status === APPLICATION_STATUS.PENDING_ADMIN_ACCEPT ? '待受理' : ''" /></header>
      <section class="card"><h2>学生信息</h2><dl class="info-grid"><div><dt>姓名</dt><dd>{{ item.studentName }}</dd></div><div><dt>学号</dt><dd>{{ item.studentId }}</dd></div><div><dt>申请人身份</dt><dd>{{ item.captainId === item.currentUserId ? '队长' : '成员' }}</dd></div><div><dt>当前流程</dt><dd>管理员受理</dd></div></dl></section>
      ...
      <section class="card reviewer-assignment"><label for="assigned-reviewer"><strong>选择审核老师 <span class="required">*</span></strong></label><select id="assigned-reviewer" v-model="selectedReviewerId"><option value="">请选择审核老师</option><option v-for="reviewer in mockReviewers" :key="reviewer.reviewerId" :value="reviewer.reviewerId">{{ reviewer.reviewerName }} · {{ reviewer.college }} · {{ reviewer.direction }}（待处理 {{ reviewer.pendingCount }} 项）</option></select><p>请从系统预设审核老师池中选择，本次受理完成后将立即分配。</p></section>
      <section class="acceptance-notice">管理员受理阶段用于分配审核老师，不在此阶段驳回申请。</section>
      <section class="card"><label for="acceptance-opinion"><strong>管理员受理意见</strong></label><textarea id="acceptance-opinion" v-model="opinion" rows="5" placeholder="请输入受理与分配说明。"></textarea><p v-if="feedback.message" class="feedback" :class="`feedback--${feedback.type}`">{{ feedback.message }}</p></section>
      <ReviewActionBar approve-text="受理并分配" :show-reject="false" :disabled="item.status !== APPLICATION_STATUS.PENDING_ADMIN_ACCEPT" @approve="acceptApplication"><template #before><button class="back-button" type="button" @click="goBack">返回</button></template></ReviewActionBar>
    </template>
