const STATUS_META = {
  pending: { text: '待处理', color: 'warning' },
  draft: { text: '草稿', color: 'default' },
  submitted: { text: '已提交', color: 'info' },
  pending_advisor: { text: '待指导老师确认', color: 'warning' },
  pending_result: { text: '待补交成果', color: 'warning' },
  waiting_result: { text: '待补交成果', color: 'warning' },
  need_supplement_result: { text: '需补交成果', color: 'danger' },
  pending_material: { text: '待学生补交成果', color: 'warning' },
  material_submitted: { text: '补交成果待指导老师确认', color: 'info' },
  extension_requested: { text: '普通延期待指导老师确认', color: 'warning' },
  extension_admin_review: { text: '特殊延期待管理员审核', color: 'warning' },
  material_overdue: { text: '成果补交已逾期', color: 'danger' },
  supplement_rejected: { text: '补交成果已驳回', color: 'danger' },
  pending_advisor_extension: { text: '延期申请待指导老师确认', color: 'warning' },
  pending_admin_special_extension: { text: '特殊延期待管理员审核', color: 'warning' },
  pending_admin: { text: '待管理员处理', color: 'warning' },
  processed: { text: '已处理', color: 'success' },
  admin_rejected: { text: '管理员不受理', color: 'danger' },
  pending_review_assignment: { text: '管理员已受理，待分配复审老师', color: 'warning' },
  pending_re_review: { text: '已分配复审老师，等待复审', color: 'warning' },
  re_review_approved: { text: '复审通过，等待管理员最终确认', color: 'success' },
  re_review_rejected: { text: '复审驳回，等待管理员最终确认', color: 'danger' },
  final_confirmed: { text: '管理员最终确认完成', color: 'success' },
  advisor_approved: { text: '指导老师已确认', color: 'success' },
  advisor_rejected: { text: '指导老师已驳回', color: 'danger' },
  pending_admin_accept: { text: '待管理员分配审核老师', color: 'warning' },
  admin_accepted: { text: '管理员已分配审核老师', color: 'success' },
  pending_reviewer: { text: '待审核老师审核', color: 'warning' },
  reviewer_approved: { text: '审核通过', color: 'success' },
  reviewer_modified_approved: { text: '修改课时并通过', color: 'success' },
  reviewer_rejected: { text: '审核驳回', color: 'danger' },
  pending_admin_final: { text: '待管理员最终确认', color: 'warning' },
  pending_review: { text: '待审核', color: 'warning' },
  reviewing: { text: '审核中', color: 'info' },
  pending_confirmation: { text: '待指导老师确认', color: 'warning' },
  pending_admin_publish: { text: '待管理员确认发布', color: 'warning' },
  published: { text: '已发布', color: 'success' },
  registration_open: { text: '报名中', color: 'success' },
  registration_closed: { text: '报名已截止', color: 'default' },
  selection_pending: { text: '待筛选成员', color: 'warning' },
  leader_pending: { text: '待指定队长', color: 'warning' },
  task_in_progress: { text: '任务进行中', color: 'info' },
  pending_admin_review: { text: '待管理员审核', color: 'warning' },
  publish_rejected: { text: '发布被驳回', color: 'danger' },
  closed: { text: '报名已截止', color: 'default' },
  selecting: { text: '待筛选报名学生', color: 'warning' },
  selected: { text: '已完成筛选', color: 'success' },
  leader_assigned: { text: '已指定队长', color: 'success' },
  in_progress: { text: '进行中', color: 'info' },
  result_submitted: { text: '成果已提交', color: 'info' },
  pending_advisor_result_confirm: { text: '待指导老师确认成果', color: 'warning' },
  advisor_result_approved: { text: '指导老师已确认成果', color: 'success' },
  advisor_result_rejected: { text: '指导老师已驳回成果', color: 'danger' },
  application_created: { text: '已用于课时申请', color: 'success' },
  finished: { text: '已完成', color: 'success' },
  pending_final_confirm: { text: '待管理员最终确认', color: 'warning' },
  pending_distribution_confirm: { text: '待管理员最终确认', color: 'warning' },
  confirmed: { text: '已确认', color: 'success' },
  pending_acceptance: { text: '待受理', color: 'warning' },
  accepted: { text: '已受理', color: 'success' },
  modified_approved: { text: '修改课时并通过', color: 'success' },
  pending_final_confirmation: { text: '待最终确认', color: 'warning' },
  final_approved: { text: '管理员最终确认通过', color: 'success' },
  final_rejected: { text: '管理员最终确认驳回', color: 'danger' },
  approved: { text: '已通过', color: 'success' },
  rejected: { text: '已驳回', color: 'danger' },
  completed: { text: '已完成', color: 'success' },
  cancelled: { text: '已取消', color: 'default' },
  enabled: { text: '已启用', color: 'success' },
  disabled: { text: '已停用', color: 'default' },
}

const STAGE_DESCRIPTIONS = {
  submitted: '申请已提交，等待处理。',
  advisor_review: '等待指导老师确认。',
  reviewer_review: '等待审核老师审核。',
  admin_review: '等待管理员处理。',
  final_confirmation: '审核已完成，等待最终确认。',
  completed: '流程已完成。',
  rejected: '申请未通过，请查看处理意见。',
  cancelled: '流程已取消。',
}

function normalizeValue(value) {
  return typeof value === 'string' ? value.trim().toLowerCase() : ''
}

export function getStatusText(status) {
  if (status === null || status === undefined || status === '') return '未知状态'
  return STATUS_META[normalizeValue(status)]?.text ?? String(status)
}

export function getStatusColor(status) {
  return STATUS_META[normalizeValue(status)]?.color ?? 'default'
}

export function getStageDescription(stage) {
  if (stage === null || stage === undefined || stage === '') return '暂无阶段信息。'
  return STAGE_DESCRIPTIONS[normalizeValue(stage)] ?? String(stage)
}
