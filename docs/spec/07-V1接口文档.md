...
| `task.status` | `draft` / `pending_publish_review` / `publish_rejected` / `published` / `registration_open` / `registration_closed` / `selection_pending` / `task_in_progress` / `closed` / `discarded` | 学院任务主表状态；报名结果、成果审核、课时认定状态分别由对应业务表表达 |
...
- 描述：指导老师在选中学生中指定一名队长。若仅一名学生，可默认该学生为队长。
...
| Body | `allocations[].hours` | number | 是 | 分配课时 |
...
| `id` | integer | 兑换申请 ID |
| `status` | string | `draft` |
| `calculated_allocations` | array[CreditAllocation] | 系统根据成员课时比例计算出的建议学分明细 |
