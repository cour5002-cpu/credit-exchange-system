...
| `advisor_teacher_id` | integer/null | 主指导老师 ID；由 `advisors` 中 `role=primary` 的关系计算返回，不等同于后续管理员分配的审核老师 |
| `status` | string | 当前状态 |
...
| `advisor_reviewed_at` | datetime/null | 指导老师完成确认时间 |
| `assigned_at` | datetime/null | 管理员最近一次分配审核老师时间 |
| `reviewer_reviewed_at` | datetime/null | 审核老师完成审核时间 |
| `final_reviewed_at` | datetime/null | 管理员最终确认时间 |
| `workflow_progress` | object | 五个主流程节点的独立完成标记和完成时间；历史节点不得仅根据当前状态判断 |
...
| `extension_days` | integer | 延期天数，按两个截止时间的正向时长向上取整 |
| `reason` | string | 延期原因 |
| `review_level` | string | `advisor` / `admin` |
| `status` | string | `pending_advisor_review` / `pending_admin_review` / `approved` / `rejected` / `closed` |
| `review_comment` | string/null | 审核意见 |
| `created_at` | datetime | 申请时间 |
| `reviewed_at` | datetime/null | 审核时间 |
| `attachments` | array | 延期申请附件；管理员专用详情接口返回 |
...
| FormData | `biz_type` | string | 是 | 附件所属业务类型，用于区分文件用途：`hour_application` 课时申请材料，`extension_request` 延期证明，`task_result` 任务成果，`credit_exchange` 兑换证明，`appeal` 申诉证明，`rule_file` 规则文件 |
...
- 描述：主指导老师查看学生补交后的成果列表。仅返回 `application_type=without_material` 且状态为 `material_submitted` 的记录；任务成果不进入本列表。
...
- 描述：主指导老师查看补交成果详情。仅适用于 `application_type=without_material`；任务成果调用本接口返回 `409`，应改用任务成果详情接口。
...
| Body | `attachment_ids` | array[integer] | 否 | 已通过附件上传接口取得的延期附件 ID |
...
| `review_level` | string | `advisor` 或 `admin` |
| `status` | string | `pending_advisor_review` 或 `pending_admin_review` |
| `application_status` | string | 原课时申请同步状态：`extension_requested` 或 `extension_admin_review` |

- 主要写入/更新表：`extension_requests`, `hour_applications`, `attachments`。
- 附件关联：使用通用附件表，提交成功后写入 `attachments.owner_type=extension_request`、`attachments.owner_id=extension_request_id`；当前数据库不使用 `extension_request_attachments` 关联表。

- 分流规则：严格计算 `requested_due_at - old_due_at`。大于 30 天时写入 `review_level=admin`、`status=pending_admin_review`；小于或等于 30 天时写入 `review_level=advisor`、`status=pending_advisor_review`。比较统一到秒精度，恰好 30 天不得进入管理员审核。
...
- 查询条件：`review_level=advisor AND status=pending_advisor_review`，并限制为当前主指导老师可操作的申请。
...
- 主要数据来源：`extension_requests`, `hour_applications`。
- 查询条件：`review_level=admin AND status=pending_admin_review`。

### 5.11.1 管理员获取特殊延期详情

- 接口名：管理员获取特殊延期详情
- 描述：管理员查看延期申请、申请学生及学生上传的延期证明附件。
- 接口方法：`GET`
- 接口相对路径：`/admin/extension-requests/{id}`
- 触发页面：A702 特殊延期审核详情
...
|---|---|---|
| `extension_request_id` | integer | 延期申请 ID |
| `hour_application_id` | integer | 关联课时申请 ID |
| `old_due_at` | datetime | 原成果截止时间 |
| `requested_due_at` | datetime | 申请延期后的截止时间 |
| `extension_days` | integer | 延期天数 |
| `review_level` | string | `advisor` / `admin` |
| `status` | string | 延期申请状态 |
| `reason` | string | 延期原因 |
| `student.id` | integer | 学生 ID |
| `student.student_no` | string | 学号 |
| `student.name` | string | 学生姓名 |
| `attachments` | array | 延期附件 |
| `attachments[].id` | integer | 附件 ID |
| `attachments[].filename` | string | 原始文件名 |
| `attachments[].url` | string | 附件查看/下载接口地址 |

- 主要数据来源：`extension_requests`, `hour_applications`, `students`, `attachments`。
- 附件查询条件：`owner_type=extension_request AND owner_id={id} AND status=active`。
...
- 主要写入表：`task_result_submissions`, `attachments`, `hour_applications`, `hour_application_members`, `application_advisors`。
- 处理规则：自动生成的课时申请使用 `application_type=task_result`、`status=submitted`，并继承任务的主指导老师关系。不得使用仅属于“无成果申请补交”的 `material_submitted` 状态。
- 兼容现有指导老师页面：该申请可直接通过 `GET /advisor/hour-applications/pending?status=submitted` 查询，响应中的 `advisor_teacher_id` 为任务主指导老师 ID。此时 `assigned_teacher_id`/`assigned_reviewer` 为空是正常的，管理员尚未分配审核老师。

### 6.15.1 指导老师查询待确认任务成果

- 接口名：指导老师查询待确认任务成果
- 描述：指导老师分页查询本人负责、等待确认的任务成果。该接口是任务成果专用兼容入口；现有指导老师课时申请待确认页面无需新增页面，可使用 `GET /advisor/hour-applications/pending?status=submitted`，并以 `application_type=task_result` 识别任务成果申请。任务成果与“无成果补交确认”仍保持状态和接口语义分离。
- 接口方法：`GET`
- 接口相对路径：`/advisor/task-result-submissions`
- 触发页面：T106 学生成果详情的待确认成果入口

请求参数：

| 位置 | 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| Query | `status` | string | 否 | 默认 `submitted`；传空值可查询全部状态 |
| Query | `page` | integer | 否 | 默认 1 |
| Query | `page_size` | integer | 否 | 默认 20，最大 100 |

响应 `data`：

| 字段 | 类型 | 说明 |
|---|---|---|
| `items` | array | 待确认任务成果列表 |
| `items[].id` | integer | `submission_id`，用于详情、确认和驳回接口 |
| `items[].task_id` | integer | 任务 ID |
| `items[].task_no` | string | 任务编号 |
| `items[].task_title` | string | 任务名称 |
| `items[].leader` | StudentSummary | 提交成果的队长 |
| `items[].summary` | string | 成果说明 |
| `items[].requested_hours` | number | 申请课时 |
| `items[].status` | string | 成果状态 |
| `items[].hour_application_id` | integer | 自动生成的课时申请 ID |
| `items[].attachment_count` | integer | 当前有效成果附件数量 |
| `items[].submitted_at` | datetime | 提交时间 |
| `page/page_size/total/pages` | integer | 分页信息 |

- 主要数据来源：`task_result_submissions`, `college_tasks`, `students`, `attachments`。
...
| `task` | CollegeTaskDetail | 任务信息 |
| `attachments` | array[AttachmentSummary] | 成果附件 |
| `versions` | array | 成果提交版本；`attachments` 仅返回当前版本附件 |

- 主要数据来源：`task_result_submissions`, `college_tasks`, `attachments`。
- 附件下载：使用 `GET /attachments/{attachment_id}?download=true`，请求必须携带登录 Session。

### 6.17 指导老师确认任务成果

- 接口名：指导老师确认任务成果

- 描述：指导老师确认任务成果，通过后进入管理员分配审核老师环节。该专用接口保留兼容；现有课时申请页面也可调用 `POST /advisor/hour-applications/{hour_application_id}/approve`，两种调用都会同步更新成果提交、任务和课时申请状态。
...
- 描述：指导老师驳回队长提交的任务成果，队长可修改后重新提交。该专用接口保留兼容；现有课时申请页面也可调用 `POST /advisor/hour-applications/{hour_application_id}/reject`，两种调用都会同步更新成果提交、任务和课时申请状态。
...
- 返回规则：`members` 始终返回原课时申请的全部有效成员，不得因为当前没有生效的结构化兑换规则而返回空数组；无规则时仅令 `can_submit=false`、`conversion_rule=null`。
...
| `target_status` | string | 普通有成果课时申请为 `submitted`；已补交过成果的无成果申请为 `material_submitted`；学分兑换通常为 `submitted` |

- 主要更新表：`appeals`, 原业务表, `operation_logs`。

- 状态变化：
  - `appeals.status`: `pending_admin_review` -> `processing`
  - 原业务：从 `appealed` 或申诉关联状态回到待指导老师再次确认状态

- 校验规则：同一业务对象只能申诉一次；管理员必须填写 `admin_advice`；申诉通过后不得直接分配审核老师，必须先由指导老师再次确认。
- 无成果复用规则：如果无成果申请已经补交成果并进入过审核老师/终审环节，申诉受理后直接复用原 `achievement_summary` 和原附件，回到 `material_submitted`；学生不需要再次补交成果。
...
- 描述：指导老师对申诉通过后的原业务再次确认。确认通过后，原业务进入管理员再次分配审核老师环节；驳回则原结果维持或流程关闭，具体按原业务状态流转处理。已补交成果的无成果申请通过补交成果确认语义处理，但直接复用原成果和附件。
